    ...
    if staticdata ~= "" then
        ...
        if not validate_staticdata(data) then
            ...
            minetest.log("warning",
                "Removed blood_splatter:canvas entity at " .. p .. " in on_activate because of invalid staticdata")
            return
        end
        ...
        for _, obj in ipairs(rivals) do
            if obj ~= self.object then
                ...
                if ent and ent.name == "blood_splatter:canvas" then
                    obj:remove()
                end
            end
        end
...
minetest.register_entity("blood_splatter:canvas", CanvasEntity)
...
local lifetime = shared.decalLifetime
function CanvasEntity:on_step(dt)
