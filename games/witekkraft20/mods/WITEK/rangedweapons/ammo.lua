...
local objects_in_radius = minetest.get_objects_inside_radius(moveresult.collisions[1].node_pos, 1.6)
				for _,object in ipairs(objects_in_radius) do
					if object:get_luaentity() ~= self and object:get_armor_groups().fleshy then
						local damage = (math.random(1, 7) + math.random(0, 10))^randomFloat(0.9, 1.35)
						--minetest.debug(tostring(damage))
						object:punch(self.object, 1.0, {full_punch_interval = 1.0, damage_groups = {fleshy=damage},}, direction)
						self.object:remove()
						minetest.sound_play("damage_hit", {pos = pos}, true)
						return true
					end
				end
...
if minetest.get_node(moveresult.collisions[1].node_pos).name == "mcl_target:target_off" then
	mcl_target.hit(moveresult.collisions[1].node_pos, 1) --10 redstone ticks
end
...
local nodeName = minetest.get_node(moveresult.collisions[1].node_pos).name
	...
	if nodeName == "mineclone:glass" then
...
minetest.sound_play("glass_break",{moveresult.collisions[1].node_pos})
	...
	if   nodeName == "mineclone:pane_flat" or
		nodeName == "mineclone:pane_natural_flat" then
minetest.swap_node(moveresult.collisions[1].node_pos, {name = "air"})
...
for i = 1,math.random(3,6) do
	minetest.add_particle({
		...
		expirationtime = math.random(6,9), 
...
damage[_] = damage[_] * 10
--minetest.debug(tostring(damage[_]))
...
OnCollision = function(player,bullet,target)
	mcl_tnt.boom(bullet.object:get_pos(), {radius = 2})
end,
ammo_particles = {
...
OnCollision = function(player,bullet,target)
	mcl_tnt.boom(bullet.object:get_pos() , {radius = 3})
end,
ammo_particles = {
	velocity = {x=1,y=1,z=1},
	acceleration = {x=1,y=1,z=1},
