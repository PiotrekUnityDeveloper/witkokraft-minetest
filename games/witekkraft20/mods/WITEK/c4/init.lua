...
local tnt_txt = {"c4:minitnt", "c4:packed_tnt"}
local all_tnt = {}
...
local function activate_if_tnt(nodename, nodepos, tntpos, tntradius)
	local explodetime_short = 6 -- seconds
...
nuke.register_tnt = function(nodename, def)
	def.explodetime = def.explodetime or 6 -- seconds
	def.tntradius = 9
	...
	minetest.register_node(nodename, {
		...
		on_burn = function(pos)
			minetest.remove_node(pos)
			nuke.spawn_tnt(pos, "c4:minitnt")
			minetest.check_for_falling(pos)
		end,
		on_punch = function(pos, node, puncher, pointed)
			local item = puncher:get_wielded_item()
			--minetest.log("none", item:get_name())
			if string.find(item:get_name(), "torch") then
				--minetest.log("none", "siema2")
				minetest.remove_node(pos)
				nuke.spawn_tnt(pos, "c4:minitnt")
				minetest.check_for_falling(pos)
			end
	...
	local entity = {
		...
		health = 10, -- number of punches required to defuse
...
nuke.register_tnt("c4:minitnt", {
	description = "C4",
	tiles = {"minitop.png","minibottom.png","miniside.png"},
	on_explode = function(pos)
	on_explode_normal(pos, 1.7)
	end,
})
